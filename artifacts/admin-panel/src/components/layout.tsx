...
import { LayoutDashboard, Users, ShoppingCart, Settings, LogOut, Menu, X, Globe, Smartphone, Newspaper, ShieldCheck, UserCheck, LayoutGrid } from "lucide-react";
...
export function AppLayout({ children, username, onLogout }: AppLayoutProps) {
  ...
  const links = [
    ...
    { href: "/accounts", label: "Telegram аккаунты", icon: UserCheck },
    { href: "/other-accounts", label: "Прочее", icon: LayoutGrid },
    { href: "/users", label: "Пользователи", icon: Globe },
    { href: "/sessions", label: "Сессии", icon: Smartphone },
    { href: "/news", label: "Новости", icon: Newspaper },
    { href: "/admins", label: "Администраторы", icon: ShieldCheck },
    ...
    <div className="flex min-h-screen w-full bg-background dark">
      ...
      <aside className="w-64 border-r border-border bg-card flex-col hidden md:flex">
        <div className="p-4 border-b border-border flex items-center gap-3">
          <img src={`${import.meta.env.BASE_URL}logo.png`} alt="Logo" className="h-8 w-8 rounded-md neon-logo" />
