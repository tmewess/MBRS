...
router.post("/balance/purchase", async (req, res): Promise<void> => {
 ...
 const [updatedAccount] = await db.select().from(accountsTable).where(eq(accountsTable.id, accountId));

 res.json({ success: true, orderId: order.id, account: updatedAccount });
});
